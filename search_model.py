...
class TinyNetwork(nn.Module):
  ...
  def __init__(self, C, N, max_nodes, num_classes, criterion, search_space, dropout, args, affine=False, track_running_stats=True):
    ...
    for index, (C_curr, reduction) in enumerate(zip(layer_channels, layer_reductions)):
      if reduction:
        ...
        cell = SearchCell(C_prev, C_curr, 1, max_nodes, search_space, dropout, affine, track_running_stats)
    ...
    self._arch_parameters = Variable(2e-1*torch.ones(num_edge, len(search_space)).cuda(), requires_grad=True)
  ...
  def forward(self, inputs, weights=None):
    sim_nn = []
    weights = self._arch_parameters if weights is None else weights
...
class TinyNetworkDarts(TinyNetwork):
  def __init__(self, C, N, max_nodes, num_classes, criterion, search_space, dropout, args,
               affine=False, track_running_stats=True):
    super(TinyNetworkDarts, self).__init__(C, N, max_nodes, num_classes, criterion, search_space, dropout, args,
